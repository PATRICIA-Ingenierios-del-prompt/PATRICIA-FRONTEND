...
function CampusMap({ events, height = 480, selectedId, onSelect, pickMode, pickedPos, onPick }: {
  events: UiEvent[]; height?: number; selectedId?: UUID | null;
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl" style={{ background: `${event.color}18` }}>{event.emoji}</div>
              <div>
...
export function EventosView({ onTrackEvent, enrolledIds, onEnroll, onLeave }: {
  ...
  const uiEvents = useMemo(() => {
    return raw.map(adapt).filter((e): e is UiEvent => e !== null)
      // Requirement #3: started events must not appear on the map.
      .filter(e => detailCache.get(e.id)?.started !== true);
  }, [raw, detailCache]);
  ...
  const detailEvent = detailId ? uiEvents.find(e => e.id === detailId) ?? null : null;
  ...
  return (
    <div className="h-full overflow-y-auto pb-6">
      ...
      <div className="rounded-2xl overflow-hidden border mb-4" style={{ borderColor: 'rgba(108,99,255,0.2)', boxShadow: '0 4px 24px rgba(108,99,255,0.08)' }}>
        <CampusMap events={filtered} selectedId={selectedId} onSelect={(id) => { setSelectedId(id); setDetailId(null); }} />
      </div>
